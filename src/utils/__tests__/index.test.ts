import { getScriptPath, checkIfHasDefaultExporting } from '../index';
...
describe('Utils', () => {
  ...
  describe('checkIfHasDefaultExporting', () => {
    it('should return true if file path has default exporting member', () => {
      const hasDefaultExporting: boolean = checkIfHasDefaultExporting('path/to/access');
      expect(hasDefaultExporting).toBe(true);
    });

    it('should return false if file path does not has default exporting member', () => {
      const hasDefaultExporting: boolean = checkIfHasDefaultExporting('path/not/exist');
